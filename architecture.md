...
vbuff is a cross-platform (macOS, Windows, Linux/X11, Linux/Wayland) clipboard manager and text-expansion tool written in Rust. This document defines the target system: durable searchable history, SQLCipher at-rest encryption, native all-flavor capture, and opt-in peer-to-peer sync. The current binary has the resident local loop, schema v7 search/CAS/migration/lifecycle hardening, fail-closed AI eligibility with local feature vectors, tiered capture recovery, byte/RSS pressure policy, secret clawback and reason-specific masking, natural recall, History/Compose/Trust/Settings surfaces, explicit config-schema migration, everyday history controls, local `ask`, an offline verifier, and tested sync/IPC/plugin/update foundations. It still uses `arboard`, stores the main SQLite database without SQLCipher, has no OS-keystore key wired to durable Undo, and has no live daemon dispatch, native integration clients, sync transport, third-party plugin runtime, or updater fetch/install path. Current-versus-target status is tracked in the [batch 001-050](docs/implementation-batch-001-050.md), [batch 051-100](docs/implementation-batch-051-100.md), [batch 101-150](docs/implementation-batch-101-150.md), [batch 151-200](docs/implementation-batch-151-200.md), [batch 201-250](docs/implementation-batch-201-250.md), [batch 251-300](docs/implementation-batch-251-300.md), and [batch 301-350](docs/implementation-batch-301-350.md) ledgers; current sharp edges live in the [public limitation ledger](docs/limitations.md).
...
### Process model: one binary, two roles, never two daemons

vbuff ships as a **single Rust binary** that can run in two roles selected at startup: a long-lived **core daemon** (clipboard watcher + store + IPC server + sync) and a **transient UI/CLI client**. This is deliberately *not* a microservice split and *not* a monolithic single-window app.
...
| `src/paste.rs` | Clipboard staging, pre-write verification, delayed reread, and paste-back sequencing |
...
| **Paste coordinator** | Fingerprint verifiable input, stage selected flavors, reread at the deadline, then permit or block paste injection | Searching, storage schema, row rendering | `src/paste.rs` |
...
9. **Done:** schema v7 owns FTS5 prose/code indexes, structured facets, SimHash/dHash plus normalized text groups, keyset search, Bloom-assisted exact dedup, transactional refcounted CAS, per-kind/collection retention, lifecycle annotations, residency/backup evidence, import/blob quarantine, versioned export, legal hold, externally keyed encrypted grace records, verified migration preflight/rollback, expiry, content audits, and eligible local embeddings. SQLCipher and the OS-keystore provider are still absent.
...
19. **Next:** collect real Wayland and dogfood evidence, connect trust/recall/lifecycle foundations through focused UI/application owners, give native backends generation/provenance/concealed/all-flavor capture plus hook re-subscribe, add SQLCipher/keystore, replace the Windows bootstrap fallback with the canonical named pipe, and move stable contracts into `vbuff-daemon` at M7.
...
`arboard` is the obvious first reach but the **wrong tool for monitoring**: no change notification, text-or-image-only single-flavor reads, no RTF/HTML/file-lists, no custom MIME, no sensitivity flags, no source-app info. **Decision:** native capture backends per OS; keep `arboard` only as an optional fallback *writer* on paste and a smoke-test crate during early bring-up. The capture layer is GUI-agnostic and has no dependency on the rendering crate.
...
A clipboard manager sees every password, OTP, API key, and private message that transits the clipboard. The threat model is the spine of the architecture: **vbuff defaults to local-only, encrypts at rest, and treats "do not capture" as a first-class, fail-closed code path that runs before anything touches disk.**
...
## Recommended crate dependency table
...
| `arboard` | Fallback clipboard *writer* on paste; early smoke tests | Insufficient for monitoring (no events, single-flavor, no concealed hints). Never the primary capture path. |
...
## Failure modes and degradation (consolidated)
...
| 301-350 | Seventh implementation/review batch complete with trust/recall runtime and foundations, schema 7 lifecycle, and native desktop evidence dependencies explicit | [Batch 301-350 ledger](docs/implementation-batch-301-350.md) |
...
23. **Migration dry-run + checksum manifest with auto-rollback** `[M]` - Before each forward migration, record a manifest (pre-version, row counts, schema hash) and run the migration against a temp copy first; on success swap atomically, on any error restore the online-backup file and surface a precise 'migration N failed at step K' instead of a half-applied DB. _Value: Forward-only migrations + a full backup is the current safety net, but a failure mid-batch still leaves the live file touched and the user with no diagnostic. A dry-run + manifest turns upgrade risk (a top way clipboard managers lose history) into a recoverable, debuggable event._
...
63. **Hardware-bound key wrapping (Secure Enclave / TPM 2.0 / TPM-backed DPAPI)** `[L]` - Add a SecretStore tier that wraps the root DEK with a non-exportable hardware key: a Secure Enclave key on macOS, a TPM 2.0 sealed key on Windows/Linux, so the DEK can only be unwrapped on this physical machine even if the keychain blob is copied. _Value: Today the DEK sits in the OS keychain, which a same-user process or a stolen keychain export can read. Hardware binding means an attacker who exfiltrates both vbuff.db and the keychain entry still cannot decrypt off-device, materially strengthening the 'stolen disk / backup leak' defense for high-value users._
64. **Biometric / passkey unlock gate (Touch ID, Windows Hello, FIDO2)** `[M]` - Let auto-lock be released by platform biometrics or a hardware passkey (LocalAuthentication on macOS, Windows Hello, libfido2 on Linux) as an alternative to the master password / PIN, releasing the in-memory DEK only after a successful local user-presence check. _Value: The current unlock story is password + rate-limited PIN, both phishable/shoulder-surfable. A user-presence biometric gate is faster and resists the most common real attack (someone sits at your unlocked-screen machine), and pairs naturally with the hardware-bound DEK idea._
...
68. **Cooperate with (and detect) OS-native clipboard history capturing our writes** `[M]` - On paste-back and on any internal copy, set the OS 'do not retain' hints we already honor on input (org.nspasteboard.ConcealedType, ExcludeClipboardContentFromMonitorProcessing / CanIncludeInClipboardHistory=false) so Win+V, macOS Universal Clipboard, and other managers don't silently mirror vbuff's decrypted output into their own plaintext stores; warn when a rival manager is detected running. _Value: vbuff encrypts its own store but the moment it writes a chosen clip back, Win+V or another manager can re-capture it into an unencrypted history, defeating the whole guarantee. Marking our own writes as non-retainable closes a leak that sits entirely outside the current threat model._
...
90. **WASM Component-Model plugins with capability-scoped WIT interfaces** `[L]` - Define a stable WIT (WebAssembly Interface Types) contract that third-party plugins compile to wasm32-wasip2 components, run in Wasmtime inside the daemon, and call only the host functions their declared capabilities grant (e.g. read-clip, transform-text, net-fetch). _Value: Solves the central tension the recommendation flags - it wants extensibility but explicitly rejects the CopyQ JS-engine 'arbitrary code on the hot path' trap. A sandboxed WASM component with no ambient authority gives third-party power without an RCE surface, and being language-agnostic (Rust/Go/JS/Zig all target wasm) it dodges committing to one embedded scripting DSL. This is the safe form of catalog #447/#449._
...
99. **Signed, reproducible plugin bundles with a lockfile** `[M]` - Distribute plugins as signed bundles (manifest + wasm + assets) verified against a publisher key on install, and record installed plugins with pinned versions and hashes in a `plugins.lock` next to the versionable config (#471) so a config sync reproduces the exact extension set on another machine. _Value: Open-source-core trust (Bet 5) extends to extensions: signature verification stops a tampered plugin from impersonating a trusted one, and a lockfile makes a user's whole setup - settings plus extensions - portable and auditable, supporting the 'export/import configuration' story (#485) end-to-end. Without pinning, a synced config that references plugins is non-reproducible._
