...
- **Storage/security:** build-time selection of bundled DuckDB (default, physical schema v1) or SQLite (schema v7), with a single owning worker and a bounded command queue. The frozen SQLite schema v7 remains an export/migration compatibility identifier. The database is not encrypted. Short-lived memory-only secret classes remain outside durable storage.
- **Migration/backup:** old SQLite is opened read-only; all copied fields and payload files are verified before the DuckDB file is published. Old SQLite and its CAS remain intact. New files use a separate CAS directory. Checkpoints do not promise secure physical erasure. [Storage backends](docs/storage-backends.md) and [DuckDB migration and ownership](docs/duckdb-migration.md) describe the current storage architecture; older SQLite/FTS target sections below are historical design context.
- **Application boundary:** `src/store_owner.rs` owns the writable connection; `src/history.rs` coordinates history policy and publishes domain events; `src/history_presenter.rs` adapts events to the GUI. CLI `ask` and `doctor` use the resident control endpoint when available.
...
| `src/paste.rs` | Guarded clipboard staging plus one-shot restore/confirm/inject sequencing |
...
| **Platform** | Clipboard/hotkey/paste traits, macOS/Windows confirmed-target delivery, plus desktop-shell, capability, security, lifecycle, access/profile/theme, and Wayland/Windows decision contracts | Product policy, SQL, visual layout | `crates/vbuff-platform/src/traits.rs`, `confirmed_paste.rs`, then `desktop.rs`, one concern under `desktop_policy/`, `capabilities.rs`, `security.rs`, or `lifecycle.rs` |
...
| **Paste coordinator** | Enforce atomic sensitive-history-exclusion before any clipboard write; stage eligible flavors; sequence hide, target restore, foreground confirmation, clipboard guard, and one-shot injection | Searching, storage schema, row rendering, native target identity | `src/paste.rs`, backed by `crates/vbuff-platform/src/confirmed_paste.rs` |
...
3. **Done/partial native:** `PasteCoordinator` owns clipboard staging, one-shot target state, restoration, delayed foreground confirmation, clipboard re-verification, and injection sequencing. A global-hotkey summon arms the target; tray/relaunch opening remains copy-only because its foreground identity is ambiguous. macOS uses `NSRunningApplication` plus Accessibility trust; Windows uses foreground `HWND` identity. Linux, remote/headless sessions, and failed proof remain copy-only; native sink-app conformance is still open.
...
19. **Next:** implement and prove native generation/provenance/concealed/all-flavor capture and OS-history exclusion; harden target confirmation with native sink-app/elevation/session evidence; then collect real Wayland and dogfood evidence. SQLCipher/keystore, the canonical Windows named pipe, and extraction of stable contracts into `vbuff-daemon` remain later gates.
