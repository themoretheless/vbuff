...
- **UI/runtime:** native desktop `eframe`/`egui` only, with History/Compose/Trust/Settings surfaces. The web demo, browser UI, WASM binary, WASM CI target, and component-model plugin path have been removed.
...
5. **Done:** `vbuff-gui::design` owns stable dimensions, spacing, theme-aware semantic colors, and font-independent icon buttons; navigation, projection, media, and Trust rendering are separate modules, while the menu-bar icon is isolated in `src/tray.rs`. Twenty-four headless WGPU goldens cover themes, DPI, and major surfaces; persisted UI preferences and OS-derived reduced motion stay outside widget-local state.
