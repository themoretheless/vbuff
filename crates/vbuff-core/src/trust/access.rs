...
const CLIP_ID_HASH_DOMAIN: &[u8] = b"vbuff-access-clip-id-v1\0";
const REASON_HASH_DOMAIN: &[u8] = b"vbuff-access-reason-v1\0";
...
impl ClipAccessRequest {
    ...
    pub fn decide(
        ...
        self.decided = true;
        Ok(AccessAuditEntry {
            clip_id_hash: domain_hash(
                CLIP_ID_HASH_DOMAIN,
                self.clip_id.to_string_repr().as_bytes(),
            ),
            integration_id: self.integration_id.clone(),
            reason_fingerprint: domain_hash(REASON_HASH_DOMAIN, self.reason.as_bytes()),
            reason_bytes: self.reason.len() as u16,
            decision,
            timestamp_ms,
        })
    }
...
impl fmt::Debug for AccessAuditEntry {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("integration_id", &"[redacted]")
            .field("reason_fingerprint", &"[redacted]")
            .field("reason_bytes", &self.reason_bytes)
...
impl AccessAuditEntry {
    ...
    pub const fn reason_fingerprint(&self) -> [u8; 32] {
        self.reason_fingerprint
    }

    pub const fn reason_bytes(&self) -> u16 {
        self.reason_bytes
    }
...
mod tests {
    ...
    fn access_requires_a_human_reason_and_produces_one_audit_decision() {
        ...
        assert_ne!(audit.reason_fingerprint(), [0; 32]);
        assert_eq!(audit.reason_bytes(), 43);
        ...
        assert!(!format!("{audit:?}").contains("Insert the selected"));
    }
}
