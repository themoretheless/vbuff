...
use vbuff_core::recall::{MatchExplanation, complete_query, parse_natural_query};
use vbuff_core::trust::EphemeralCountdown;
...
pub struct PopupApp {
    ...
    query_completion_selected: usize,
    /// Query value whose completion popup was explicitly dismissed.
    dismissed_completion_query: Option<String>,
    /// One-shot request to reveal a keyboard-selected virtual row.
...
impl PopupApp {
    ...
    /// Apply preferences loaded by the native composition layer.
    ...
    /// Reconcile native viewport visibility with the popup state machine.
    pub fn request_hide(&mut self, ctx: &egui::Context) {
        self.hide(ctx);
    }
...
impl eframe::App for PopupApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let (show_requested, hidden_announcement, hidden_announcement_revision) = {
            let Ok(mut state) = self.state.lock() else {
                tracing::error!("GUI state mutex poisoned");
                return;
            };
            (
                std::mem::take(&mut state.show_requested),
                state.accessibility_announcement.clone(),
                state.announcement_revision,
            )
        };
        if show_requested {
            self.show(ctx);
        ...
        // 2. Snapshot visible state. Clip payloads are shared through Arc.
        let (
            clips,
            paused,
            pause_reason,
        ...
        let query_completions = if self.surface == PopupSurface::History
            && focused_widget == Some(history_search_id())
            && self.dismissed_completion_query.as_deref() != Some(self.query.as_str())
        ...
        self.query_completion_selected = self
            .query_completion_selected
            .min(query_completions.len().saturating_sub(1));
        let query_completions_active = !query_completions.is_empty();
        let selected_delivery_allowed = filtered
            .get(self.selected)
            .and_then(|hit| clips.iter().find(|clip| clip.id == hit.id))
            .is_some_and(|clip| self.delivery.allows(clip.meta.sensitive));
        let history_shortcuts_enabled = self.surface == PopupSurface::History
            ...
            && !self.feedback_preview
        {
            ctx.input(|i| {
                ...
                if !self.command_palette_open && modifier_down && i.key_pressed(Key::K) {
                ...
                if history_shortcuts_enabled && i.key_pressed(Key::ArrowDown) && total > 0 {
                    self.selected = (self.selected + 1).min(total - 1);
                    self.scroll_selection_into_view = true;
                }
                if history_shortcuts_enabled && i.key_pressed(Key::ArrowUp) && total > 0 {
                    self.selected = self.selected.saturating_sub(1);
                    self.scroll_selection_into_view = true;
                }
                if history_shortcuts_enabled && i.key_pressed(Key::Home) && total > 0 {
                    self.selected = 0;
                    self.scroll_selection_into_view = true;
                }
                if history_shortcuts_enabled && i.key_pressed(Key::End) && total > 0 {
                    self.selected = total - 1;
                    self.scroll_selection_into_view = true;
                }
                if history_shortcuts_enabled && i.key_pressed(Key::PageDown) && total > 0 {
                    self.selected = (self.selected + keyboard_page).min(total - 1);
                    self.scroll_selection_into_view = true;
                }
                if history_shortcuts_enabled && i.key_pressed(Key::PageUp) && total > 0 {
                ...
                if history_shortcuts_enabled && i.modifiers.alt && total > 0 {
        ...
        if dismiss_completions {
            self.dismissed_completion_query = Some(self.query.clone());
        }
        if self.query_completion_selected != completion_before_keys
            && let Some(completion) = query_completions.get(self.query_completion_selected)
            && let Ok(mut state) = self.state.lock()
        {
            state.announce(format!("Search suggestion: {completion}"));
        }
        if let Some(completion) = accepted_completion {
        ...
        if self.selected != selected_before_keys
            && let Some(hit) = filtered.get(self.selected)
            && let Some(clip) = clips.iter().find(|clip| clip.id == hit.id)
        {
            let preview = row_preview_with_peek(clip, false);
        ...
        if self.preferences != preferences_before {
            self.actions.push_back(UiAction::SetUiPreferences {
                ...
                reduced_motion_changed: self.preferences.reduced_motion
                    != preferences_before.reduced_motion,
            });
        }
...
impl PopupApp {
    ...
    fn render_action_menu(&mut self, ui: &mut egui::Ui, paused: bool) {
        ...
        let _ = egui::Popup::menu(&response).width(190.0).show(|ui| {
            ...
            if self.surface == PopupSurface::Trust && ui.button("Feedback report").clicked() {
    ...
    fn render_query_completions(&mut self, ui: &mut egui::Ui, anchor: &egui::Response) {
        ...
        if self.dismissed_completion_query.as_deref() == Some(self.query.as_str()) {
            return;
        }
        ...
        let completion_count = completions.len().min(5);
        let _ = egui::Popup::from_response(anchor)
            ...
            .show(|ui| {
                ...
                ui.ctx().accesskit_node_builder(ui.id(), |node| {
                    node.set_role(egui::accesskit::Role::ListBox);
                    node.set_label("Search suggestions");
                    node.set_size_of_set(completion_count);
                });
                for (index, completion) in completions.into_iter().take(5).enumerate() {
                    let selected = index == self.query_completion_selected;
                    let response = ui.selectable_label(selected, &completion);
                    ui.ctx().accesskit_node_builder(response.id, |node| {
                        node.set_role(egui::accesskit::Role::ListBoxOption);
                        node.set_label(completion.clone());
                        node.set_selected(selected);
                        node.set_position_in_set(index + 1);
                        node.set_size_of_set(completion_count);
                    });
                    if selected {
                        active_option = Some(response.id);
                    }
                    if response.clicked() {
        ...
        ui.ctx().accesskit_node_builder(anchor.id, |node| {
            node.set_role(egui::accesskit::Role::EditableComboBox);
            node.set_label("Search clipboard history");
            node.set_expanded(true);
            if let Some(active) = active_option {
                node.set_active_descendant(active.value().into());
            }
        });
    ...
    fn render_hotkey_coachmark(&mut self, ui: &mut egui::Ui, hotkey: &str) {
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    design::status_dot(ui, design::info(ui));
    ...
    fn render_settings_surface(
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                design::section_heading(ui, "Desktop", None);
                let mut start_at_login = launch_at_login;
                if ui
                    .checkbox(&mut start_at_login, "Start vbuff at login")
                    .changed()
                {
                    self.actions
                        .push_back(UiAction::SetLaunchAtLogin(start_at_login));
                }
                ui.label(
                    RichText::new("Background launches stay in the menu bar.")
                        .small()
                        .weak(),
                );
    ...
    fn render_command_palette(
        ...
        let entries = vec![
            ...
            (
                ...
                "Peek selected clip".to_owned(),
                true,
            ),
            (
                PaletteCommand::ToggleCapture,
                if paused {
                    "Resume capture".to_owned()
                } else {
                    "Pause capture".to_owned()
                },
                true,
            ),
            (
                PaletteCommand::TogglePreview,
                "Toggle large preview".to_owned(),
                true,
            ),
            (
                PaletteCommand::ToggleMotionInspector,
                "Toggle motion inspector".to_owned(),
                true,
            ),
        ];
        let mut chosen = None;
        ...
        let response = egui::Modal::new(egui::Id::new("vbuff_command_palette")).show(ctx, |ui| {
            ...
            let search = ui.add_sized(
                ...
                egui::TextEdit::singleline(&mut self.command_query)
                    .id_source("vbuff_command_search")
                    .hint_text("Command"),
            );
            ctx_accessible_name(ui.ctx(), search.id, "Search commands");
            ...
            for (command, label, enabled) in entries {
                if query.is_empty() || label.to_lowercase().contains(&query) {
                    if enabled {
                        first_visible.get_or_insert(command);
                    }
                    if ui
                        .add_enabled(enabled, egui::Button::selectable(false, label))
                        .on_disabled_hover_text(
            ...
            if search.has_focus() && ui.input(|input| input.key_pressed(Key::Enter)) {
                chosen = first_visible;
            }
        });
        ctx_accessible_dialog(ctx, response.response.id, "Command palette", false);
        if let Some(command) = chosen {
            self.execute_palette_command(command, filtered, clip_by_id, memory_only_clips);
            self.command_palette_open = false;
    ...
    fn render_action_flyout(
        ...
        let memory_only = memory_only_clips.contains(&id);
        ...
        let response = egui::Modal::new(egui::Id::new("row_action_flyout")).show(ctx, |ui| {
            ...
            if ui
                .add_enabled(
                    self.delivery.allows(clip.meta.sensitive),
                    egui::Button::new(self.delivery.action_label()),
                )
            ...
            if !memory_only
                && clip.primary_text().is_some()
                && ui.button("Create plain-text clone").clicked()
            {
        ...
        ctx_accessible_dialog(ctx, response.response.id, "Clip actions", false);
        ...
        if request_delete {
            ...
            self.selected = 0;
            self.execute_palette_command(command, &selected, clip_by_id, memory_only_clips);
    ...
    fn render_feedback_preview(&mut self, ctx: &egui::Context, capabilities: &[CapabilityView]) {
        ...
        ctx_accessible_dialog(ctx, response.response.id, "Feedback report preview", false);
    ...
    fn render_notice(&mut self, ui: &mut egui::Ui, notice: &CommandNotice) {
        let accent = match notice.level {
            NoticeLevel::Info => design::info(ui),
            NoticeLevel::Warning => design::warning(ui),
            NoticeLevel::Error => design::danger(ui),
        };
        let fill = Color32::from_rgba_unmultiplied(accent.r(), accent.g(), accent.b(), 24);
    ...
    fn render_preview_pane(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, clip: &Clip) {
            ...
            && let Some(color) = parse_hex_color(canonical.trim())
        {
            ...
            if ui
                .button(format!("{} color", self.delivery.action_label()))
                .clicked()
            {
                self.actions.push_back(UiAction::PasteText {
                    text: output,
    ...
    fn render_clear_history_confirmation(&mut self, ctx: &egui::Context) {
        ...
        ctx_accessible_dialog(ctx, response.response.id, "Clear clipboard history", true);
    ...
    fn render_delete_confirmation(&mut self, ctx: &egui::Context) {
        ...
        ctx_accessible_dialog(ctx, response.response.id, "Delete clip", true);
    ...
    fn render_profile_confirmation(&mut self, ctx: &egui::Context, current_items: usize) {
        ...
        ctx_accessible_dialog(ctx, response.response.id, "Apply profile defaults", false);
...
fn render_capture_status(
    ...
    let (label, color) = if paused {
        ...
        let color = match health {
            ...
            | CaptureHealth::SelfTestFailed => design::danger(ui),
        };
        (health.label(), color)
    };
...
fn badge_color(ui: &egui::Ui, badge: ClipBadge) -> Color32 {
    match badge {
        ClipBadge::Verified => design::success(ui),
        ClipBadge::Lossless => design::info(ui),
        ClipBadge::Partial => design::warning(ui),
...
fn history_search_id() -> egui::Id {
    egui::Id::new("vbuff_history_search")
}

fn ctx_accessible_name(ctx: &egui::Context, id: egui::Id, name: &'static str) {
    ctx.enable_accesskit();
    ctx.accesskit_node_builder(id, |node| node.set_label(name));
}

fn ctx_accessible_dialog(ctx: &egui::Context, id: egui::Id, name: &'static str, alert: bool) {
    ctx.enable_accesskit();
