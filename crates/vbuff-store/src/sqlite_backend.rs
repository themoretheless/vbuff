...
impl Store {
    ...
    pub fn open_profiled(path: &Path) -> Result<(Self, StoreOpenProfile)> {
        ...
        let database_open_ms = elapsed_ms(sqlite_started.elapsed());
    ...
    fn apply_migrations(conn: &Connection) -> Result<()> {
        ...
        // Every insert enforces hard expiry. Without this partial index an
        // ordinary copy scans the entire history even when no item has a TTL.
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_clips_expiry ON clips(expires_at) WHERE expires_at IS NOT NULL",
            [],
        )?;
    ...
    /// scan, so SQLite's case folding cannot drop a core match.
    pub fn recall_candidates_batch(
        ...
        let mut values = vec![
            ...
            Value::Integer(before_seq.unwrap_or(i64::MAX)),
        ];
        ...
        if !text.is_empty() && text.is_ascii() {
            ...
            for (index, fragment) in fragments.iter().enumerate() {
                ...
                sql.push_str("c.item_text LIKE ? ESCAPE '\\'");
                values.push(Value::Text(format!("%{}%", escape_like(fragment))));
            }
            sql.push_str(" OR c.source_app LIKE ? ESCAPE '\\'");
            ...
            sql.push_str(" OR length(CAST(c.item_text AS BLOB)) != length(c.item_text) OR length(CAST(c.source_app AS BLOB)) != length(c.source_app)
                OR (length(CAST(c.item_text AS BLOB)) < c.byte_size)");
            for kind in ContentKind::ALL {
                if kind.label().to_lowercase().contains(&text) {
                    sql.push_str(" OR c.kind = ?");
                    values.push(Value::Integer(kind.stored_discriminant()));
                }
            }
            sql.push(')');
        }
    ...
    pub fn search_health(&self) -> Result<SearchHealth> {
        ...
        let projection_rows = query_count(&self.conn, "SELECT COUNT(*) FROM clip_fts_prose")?;
    ...
    pub fn doctor(&self) -> Result<StoreDoctorReport> {
        ...
        Ok(StoreDoctorReport {
            ...
            validation_scope: "SQLite quick_check and foreign keys".into(),
            schema_version,
