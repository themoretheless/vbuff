...
Frozen on 2026-07-20 for store schema 6. The executable oracle is `tests/data_contract_freeze_v2.rs`; schema-5 migration and disk behavior are covered in `crates/vbuff-store/tests/integration.rs`. Contract v1 remains unchanged in [data-contract-v1.md](data-contract-v1.md).

## Schema 6 lifecycle additions

| Surface | v2 value | Compatibility rule |
|---|---|---|
| Current schema | `vbuff_store::SCHEMA_VERSION == 6`; v1 remains recorded as `DATA_CONTRACT_V1_SCHEMA_VERSION == 5`. | Schema 5 migrates forward through backup, dry-run, row-count/quick-check verification, and rollback. A schema-5 binary rejects schema 6 as newer; it must not downgrade or wipe it. |
