...
| 2 | Visual design, accessibility, and responsive behavior | Added one restrained action menu, stable icon controls/tooltips, adaptive density, wide preview, compact collapse, match confidence, recency, peek, color ring, Settings, command palette, and live announcements. Browser review at desktop and compact sizes exposed a WASM `std::time::Instant` panic and a DPR-dependent breakpoint; timing moved to `web-time`, native recovery stayed cfg-scoped, and CSS logical viewport dimensions now drive responsive layout. `eframe` was confirmed to own DPR scaling, so no artificial zoom override was added. |
