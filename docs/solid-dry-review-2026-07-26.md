...
3. ✅ **Сделано 2026-08-01** (вместе с пунктом 7). Секретный домен целиком в `core/secret.rs`; две OTP-эвристики слиты в объединение (не в пересечение), пороги разделены на два имени с одним домом.
   ...
   **Закрытая утечка.** Гейт захвата и детектор использовали РАЗНЫЕ списки OTP-маркеров: у гейта не было `"one-time"`. Поэтому `"Your one-time password is 483920"` захватывался как НЕ секретный — синхронизировался, был доступен ИИ, без TTL — хотя `detect_secrets` оценивал его в 0.96. Зеркально, голый шестизначный код маскировался гейтом, но был невидим для реклассификации в store.
