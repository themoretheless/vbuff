...
- Global-hotkey summons on local macOS and Windows sessions now have one-shot target-confirmed delivery: capture the active application before popup focus, hide, restore, re-confirm, verify the staged clipboard, then inject Cmd/Ctrl+V. macOS requires Accessibility permission; tray/relaunch opening, current Linux, remote/headless sessions, and any unproven target are copy-only. Sensitive copy remains blocked without proven OS-history exclusion, and native sink-app conformance evidence is still required.
...
- `vbuff-platform` (currently traits, generic `arboard` text-or-image polling/write, macOS/Windows confirmed-target paste adapters, and capability decisions; native clipboard privacy/fidelity proof remains milestone work)
...
- `vbuff-ipc` - the framed control-socket protocol and client/server; deferred to M7. Browser/editor/Vim/automation/MCP/launcher/terminal/webhook plus stable operation, completion, replay, headless, health, backup, fixture, rate-limit, and dry-run contracts are bounded foundations only. The MVP still needs a single-instance guard (bind-or-forward) but its forwarded intent is minimal (just "show popup"), not the full IPC verb surface.
...
- `vbuff-sync` runtime integration - the crate contains tested CRDT/HLC, envelope, membership, policy, anti-entropy, recovery, receipt, capability, padding, device-experience, and team-governance foundations. It stays disconnected and frozen. Only the explicit M9 handoff may activate after M6 demand; team sharing, replication, and transports require a separate M11 promotion and threat model.
- `vbuff-plugin` runtime integration - manifests, a bounded native executable protocol, typed transforms, migrations, offline evidence, signed packs/actions, test evidence, fetch/marketplace policy, failure supervision, adapters, and curated recipes are contract foundations only. No process host, socket, or sandbox is active; execution, installation, network access, publisher trust, and host-side capability enforcement remain release-gated.
