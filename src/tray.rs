...
impl Tray {
    pub(crate) fn new() -> anyhow::Result<Self> {
        ...
        let builder = TrayIconBuilder::new()
            ...
            .with_tooltip(ResidentStatus::Active.tooltip())
    ...
    pub(crate) fn acknowledge_paste(&self, now: Instant) {
        ...
        self.apply_status(ResidentStatus::PasteConfirmed);
    }
    ...
    fn apply_status(&self, status: ResidentStatus) {
        ...
        if let Err(error) = self.icon.set_tooltip(Some(status.tooltip())) {
...
fn build_icon_rgba(status: ResidentStatus) -> Vec<u8> {
    ...
    for y in 0..SIZE {
        for x in 0..SIZE {
            ...
            let check = matches!(
                status,
                ResidentStatus::Active | ResidentStatus::PasteConfirmed
            ...
            let confirmed = status == ResidentStatus::PasteConfirmed
                && ((x == 27 && (5..=9).contains(&y)) || (y == 7 && (25..=29).contains(&x)));

            let color = if confirmed {
                [0xff, 0xc8, 0x4a, 0xff]
            } else if check {
                if status == ResidentStatus::PasteConfirmed {
                    [0xff, 0xc8, 0x4a, 0xff]
                } else {
                    [0x42, 0xd3, 0xb2, 0xff]
                }
...
mod tests {
    ...
    fn status_variants_change_pixels_without_changing_dimensions() {
        ...
        let active_alpha: Vec<_> = active.chunks_exact(4).map(|pixel| pixel[3]).collect();
        let confirmed = build_icon_rgba(ResidentStatus::PasteConfirmed);
        let confirmed_alpha: Vec<_> = confirmed.chunks_exact(4).map(|pixel| pixel[3]).collect();
        assert_ne!(confirmed_alpha, active_alpha);
    }
